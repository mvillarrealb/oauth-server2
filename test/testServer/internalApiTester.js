    ...
    describe(`When making an HTTP CRUD to ${endPoint}`,function() {
      ...
      it("Should be able to validate errors",function(done){
        ...
        it("Shouldn't be able to find a non registered id",function(done){
          ...
          request.get(`${baseUrl}${endPoint}/${options.mockId}`)
          .set(headers)
          .end(function(err,res) {
            expect(err).to.be.ok;
        ...
        it("Shouldn't be able to fetch non numeric ids",function(done){
          request.get(`${baseUrl}${endPoint}/${options.badId}`)
          .set(headers)
          .end(function(err,res) {
            expect(err).to.be.ok;
            done();
        ...
        it(`Should be able to find ${baseUrl}${endPoint}/${options.elementId}`,function(done){
          request.get(`${baseUrl}${endPoint}/${options.elementId}`)
          .set(headers)
          .end(function(error,response){
            if(error){
              console.log(response.text);
        ...
        it("Shouldn't be able to find a non registered id",function(done){
          ...
          request.delete(`${baseUrl}${endPoint}/${options.mockId}`)
          .set(headers)
          .end(function(err,res) {
            expect(err).to.be.ok;
            expect(res).to.have.property("status",404);
        ...
        it("Shouldn't be able to fetch non numeric ids",function(done){
          request.delete(`${baseUrl}${endPoint}/${options.badId}`)
          .set(headers)
          .end(function(err,res) {
            expect(err).to.be.ok;
            done();
        ...
        it("Should be able to delete the requested id",function(done){
          request.delete(`${baseUrl}${endPoint}/${options.elementId}`)
          .set(headers)
          .end(function(err,res) {
            if(err){
        ...
        it("Should return a list of entities",function(done){
          ...
          .end(function assert(err,res){
            ...
            //Omited this case because we generate another insert to get tokens so... the count does not match
